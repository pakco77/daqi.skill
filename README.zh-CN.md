...
- **营地账本 `POOL.md`**：需求池。痛点 → 点子；开始干就进马厩，没有多余的中间态。
- **马厩 `SHELF.md`**：正在被执行的点子——在跑 / 松了 / 歇马，每条可删除（二次确认）。
- **这票到哪了 `NOW.md`**：每个项目唯一的热状态——目标、已验证、下一步、完成条件。
- **你的档案 `SELF.md`**：只记会改变协作方式的稳定偏好，最多 12 条。
- **营地页 `~/.daqi/camp.html`**：离线、自包含的单页营地。灰阶点阵档案风格，昼夜两套配色；火焰颗粒、马匹微动、风掠地面；弹窗内滚动、X 删除、扫描分页切换。全营只有火有颜色。营地页经本地桥（`127.0.0.1:8799`）可以请求复写：扫本地 Agent → 勾选 → 浅读/深读 → 提交到账本/马厩，全程确认制；深读没有 API key 时用本机安装的 Agent 执行。
...
达奇：我发现……     痛点记成情报
达奇：立项          计划成熟才出发
达奇：进营地        营地一眼看全，并直接打开（生成 ~/.daqi/camp.html）
