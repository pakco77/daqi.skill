...
`我发现…` 是痛点或观察，进「情报」；`我想做…` 是意图，进「点子」；证据让点子长成「计划」；只有你确认「立项 / 出发」才立项。旧的 `秘书` / `mishu` 称呼已不再触发。
...
达奇把营地渲染成一个桌面优先的单视窗：点击左侧「营地账本」查看情报/点子/计划——账本内的「扫描」入口扫各 Agent 历史、找点子和项目（勾选候选、逐项进度、复制命令）；点击右侧「马厩」查看项目、最近 Agent 与「这票到哪了」，点击中央「火」查看 `SELF.md` 中你确认过的档案。账本不显示 Agent；Agent 只属于马厩。向下滚轮或按 `Esc` 会严格返回上一层，不会一次跳过两层。

弹窗组件遵循 Pixel UI：3px 硬边框、直角、右下硬色块投影、棋盘选中态，随昼夜切换米白/墨黑两套单色（全营只有火有颜色）；面板内可滚动，面板头自带「← 返回」。场景里火有颗粒位移与火星，马匹有呼吸与耳朵微动，风掠过地面（断续风线）与空气（浮尘）。

页面会按浏览器本地时区自动在 06:00–18:00 使用白天场景，其余时间使用夜景，也可以手动切换并在刷新后保留。场景采用「Mono Dither UI / 灰阶点阵档案」：夜景为 1-bit Floyd–Steinberg并压低周边亮度，日景为 4 级 Bayer 4×4；轻面板使用米白底、1px 边框、黑白反转与小块点阵选中态。火焰有橙黄分层、点阵火光和上升烟炊，树梢受风轻摆、马匹有低频微动；所有环境动效都尊重 `prefers-reduced-motion`。
...
从马厩定位项目真实路径，只读盘点后给出极简 move plan（补骨架 + 高置信度移动 + 待判断清单），你确认后才执行；每次移动写入 `90_History/cleanup-log.md` 搬运日志，永不删除。
...
扫描 DSH / Claude Code / Codex 的会话元数据——只读 cwd 与时间戳，**不读对话**；列出工作区候选后，单选/多选按需读上下文（`shallow` 免费启发式，`deep` 走 DeepSeek 大脑，在 `~/.daqi/config.json` 填 API key，丰俭由人）。提炼出的点子和项目先给你过目，`--commit` 确认后才写入账本和马厩。全程进度可见：聊天打进度线，`~/.daqi/scan.html` 显示进度条与阶段可视化（自动刷新）。
