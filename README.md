...
The page uses the browser's local timezone: 06:00–18:00 resolves to the day scene, otherwise the night scene. Day/night can also be selected manually and survives reloads. Its **Mono Dither UI / Grayscale Dither Archive** system pairs a locally dimmed 1-bit Floyd–Steinberg night scene with a four-level Bayer 4×4 day scene, compact cream paper panels, 1px borders, inverse hovers, and small checker selection marks. The fire has layered amber flame, dithered firelight, and rising smoke; treetops sway and the horse shifts subtly. Every ambient animation honors `prefers-reduced-motion`.
