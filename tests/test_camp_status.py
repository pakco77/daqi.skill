...
def check_counts_and_readonly() -> None:
    ...
    assert "痛点发现 2 · 点子 1 · 计划 1" in result.stdout
    assert "在跑 1 · 松了 0 · 歇马 1" in result.stdout
    assert "只读" in result.stdout

    html = out.read_text()
    assert "营地账本" in html
    assert "痛点发现＝痛点 · 点子＝想法 · 计划＝待拍板" in html
...
def check_empty() -> None:
    ...
    result = run("--store", str(store))
    assert result.returncode == 0, result.stderr
    assert "痛点发现 0 · 点子 0 · 计划 0" in result.stdout
...
def check_english() -> None:
    ...
    result = run("--store", str(store))
    assert result.returncode == 0, result.stderr
    assert "痛点发现 1 · 点子 1 · 计划 1" in result.stdout
    assert "在跑 1 · 松了 0 · 歇马 1" in result.stdout
