...
- Motion: scene focus uses a continuous GPU transform; do not use `steps()` for camera movement. Panel, flame, dithered ember light, and smoke retain controlled frame character. Clipped overlay layers add restrained tree-wind and horse movement without moving the mountains or UI. `prefers-reduced-motion` removes camera interpolation and every ambient loop. No CRT scanlines, glitch, RGB split, or soft cinematic gradients.
