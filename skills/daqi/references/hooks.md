...
2. Check schema before interpreting stages. `schema_version: 3` uses `intel/idea/plan`. No version, or `schema_version: 1` / `2` (the old mishu-era camp), means legacy: preview `seed → idea`, `signal → plan`, and `candidate → plan`; a legacy `signal` that is a raw observation with no direction maps to `intel`. Ask before rewriting the legacy POOL.
...
5. Advance an idea to `plan` only when evidence exists and the intended user, visible deliverable, and next test are all clear.
6. Record only:
   - stage;
   ...
   - smallest next probe.
7. Never create a project folder or SHELF row without explicit `出发 / promote` approval.
